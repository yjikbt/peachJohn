...
//        addInfoBtn()
        ...
        //途中で停止するかもしれない効果音
        let alertSound = NSURL(fileURLWithPath: NSBundle.mainBundle().pathForResource("drum", ofType: "wav")!)
//        let _:NSError?
        do {
...
//            audioPlayer = nil
    ...
    func addGirlName(){
        guard let topGirlStr = ud.objectForKey("topGirl") as? String,
        centerGirlStr = ud.objectForKey("centerGirl") as? String,
        bottomGirlStr = ud.objectForKey("bottomGirl") as? String else {
            return
        }
        ...
        for var i = 0; i < 3; i++ {
            ...
            //ユーザデフォルトから名前を取得
            if i == 0 {
                girlNameBtn.text = topGirlStr
            } else if i == 1 {
                girlNameBtn.text = centerGirlStr
            } else if i == 2 {
                girlNameBtn.text = bottomGirlStr
            }
    ...
    func switchSetting(){
        ...
        if isSetting {//設定終了
            ...
            //回転アニメーション
            let rotate = SKAction.rotateByAngle(-0.2, duration: 0.1)
            let loop = SKAction.repeatActionForever(rotate)
            settingBtn.runAction(loop)
            
            //効果音
            settingBtn.runAction(settingOnSoundAction)
            
            let scale = SKAction.scaleTo(1.15, duration: 0.8)
            let unscale = SKAction.scaleTo(1.0, duration: 0.8)
            let scaleSeq = SKAction.sequence([scale,unscale])
            
            let loop2 = SKAction.repeatActionForever(scaleSeq)
    ...
    func popActions(){
        if isMoving {
            //ポップアニメーション
            let scale = SKAction.scaleTo(1.25, duration: 0.15)
            let unscale = SKAction.scaleTo(1.0, duration: 0.15)
            let wait = SKAction.waitForDuration(0.2)
            let startEnlarging = SKAction.runBlock {
                if self.isTouching {
            ...
            let popAction = SKAction.sequence([scale,unscale,wait,startEnlarging])
            //アクション実行
    ...
    func enlargeActions(){
        let scale = SKAction.scaleTo(2.0, duration: 2.0)
        let fadeOut = SKAction.fadeAlphaTo(0.5, duration: 2.0)
        let enlarge = SKAction.resizeToHeight(CGRectGetHeight(self.frame) / 2, duration: 2.0)
        let whitening = SKAction.colorizeWithColor(UIColor.whiteColor(), colorBlendFactor: 1.0, duration: 2.0)
        let nextAction = SKAction.runBlock {
            //タッチを止めても、もとには戻れない
        ...
        let disappear = SKAction.fadeAlphaTo(0, duration: 0.8)
        
        //ボタンのアクションを設定
        let btnGroup = SKAction.group([scale,fadeOut])
        let btnSeq = SKAction.sequence([btnGroup,nextAction])
        
        //ボタン背景のアクションを設定
        let bgGroup = SKAction.group([enlarge,whitening])
    ...
    func finishActions(){
        let enlarge = SKAction.resizeToHeight(CGRectGetHeight(frame) * 2, duration: 0.5)
        let burn = SKAction.scaleTo(10.0, duration: 0.5)
        let fadeOut = SKAction.fadeAlphaTo(0, duration: 0.2)
        let dimming = SKAction.colorizeWithColor(SKColor(red: 0.94, green: 0.94, blue: 0.94, alpha: 1.0), colorBlendFactor: 1.0, duration: 1.0)
        let transitionImagineScene = SKAction.runBlock {
            //ユーザデフォルト更新
            ...
            //妄想シーンに移動
            let fadeTransition = SKTransition.fadeWithColor(SKColor(red: 0.94, green: 0.94, blue: 0.94, alpha: 1.0), duration: 1.0)
            let imagineScene = ImagineScene(size:self.size)
        ...
        //ボタンのアクションを設定
        let btnGroup = SKAction.group([burn,fadeOut])
        
        //ボタン背景のアクションを設定
        let bgSeq = SKAction.sequence([enlarge,dimming,transitionImagineScene])
    ...
    override func touchesBegan(touches: Set<UITouch>, withEvent event: UIEvent?) {
        ...
        for touch in touches {
            ...
            if (touchedNode.name == "girlNameRect0" || touchedNode.name == "girlNameRect1" || touchedNode.name == "girlNameRect2") && (isSetting == false) {//正規表現で判定したいけど、かえって冗長になりそう
                ...
                let girlNameBtn: SKLabelNode = childNodeWithName("girlName" + indexStr) as! SKLabelNode
                ...
                var indexStr:String!
                indexStr = touchedNode.name?.componentsSeparatedByString("girlNameRect")[1]
                let girlNameBtn = childNodeWithName("girlName" + indexStr) as! SKLabelNode
                
                //alertコントローラ
                let alertController = UIAlertController(title: girlNameBtn.text! + "を編集中", message: "新しい女の子の名前を入力して下さい", preferredStyle: UIAlertControllerStyle.Alert)
                ...
                //infoシーンに移動
                let revealTransition = SKTransition.revealWithDirection(SKTransitionDirection.Up, duration: 0.5)
                let infoScene = InfoScene(size:self.size)
                
                touchedNode.runAction(infoOnSoundAction)
                
                view?.presentScene(infoScene, transition: revealTransition)
            }
    ...
    override func touchesEnded(touches: Set<UITouch>, withEvent event: UIEvent?) {
        ...
        for touch in touches {
            ...
            if isEnlarging {
                ...
                let reduce = SKAction.resizeToHeight(140, duration: 0.1)
                let dimming = SKAction.colorizeWithColor(self.backgroundColor, colorBlendFactor: 1.0, duration: 0.1)
                
                //ボタン背景のアクションを設定
                let bgGroup = SKAction.group([reduce,dimming])
    ...
    override func update(currentTime: NSTimeInterval) {
        for var i = 0; i < 3; i++ {
            ...
            let girlNameBtn = childNodeWithName("girlName" + String(i)) as! SKLabelNode
